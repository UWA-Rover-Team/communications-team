...
namespace ChunkData
{
    ...
    class ChunkData
    {
        static async Task Main(string[] args)
        {
            ...
            var enabledChunkFeatures = new List<string>();
            ...
            try
            {
                ...
                var entryNames = features.ChunkSelector.EnumEntriesByName.Keys;
                // disable all chunk data
                foreach (var entryName in entryNames)
                {
                    features.ChunkSelector = entryName;

                    // For some legacy cameras, the feature "ChunkEnable" is read-only.
                    if (features.ChunkEnable.IsWriteable())
                    {
                ...
                foreach (var entryName in new string[] { "Timestamp", "Width", "Height", "ExposureTime" }.Where(entryName => entryNames.Contains(entryName)))
                {
                    features.ChunkSelector = entryName;

                    // For some legacy cameras, the feature "ChunkEnable" is read-only and always enabled.
                    if (features.ChunkEnable.IsWriteable())
                    {
                        features.ChunkEnable = true;
                    }

                    enabledChunkFeatures.Add($"Chunk{entryName}");
                }
                    ...
                    frame.AccessChunkData(featureContainer =>
                                          {
                                              ...
                                              var chunkDetails = String.Join(", ", enabledChunkFeatures.Select(s => $"{s}: {chunkFeatures[s]}"));

                                              Console.WriteLine($"Received completed frame with {chunkDetails}");
                                          }
                                         );
                }
